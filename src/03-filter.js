...
function getSongsBySaib(songs) {
  const result = songs.filter((song) => song.artist === "Saib")
  return result;
}
 ...
 */
 ...
 function getSongsOverThreeMinutes(songs) {
  const result = songs.filter((song) => song.runtimeInSeconds > 180)
  return result;
}
 ...
 */
 ...
 function getTitleTracks(songs) {
  const result = songs.filter((song) => song.title === song.album)
    return result;
}
