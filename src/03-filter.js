...
function getSongsBySaib(songs) {
  ...
  const filterResult = songs.filter(song => song.artist === "Saib");
  return filterResult;
}
...
const filtered = songs.filter(song => song.runtimeInSeconds > 180);
return filtered;
} 
...
function getTitleTracks(songs) {
  const filterTitle = songs.filter(song => song.title === song.album);
return filterTitle;
}
