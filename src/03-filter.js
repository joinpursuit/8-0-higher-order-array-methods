...
function getSongsBySaib(songs) {
  let saibSongs = songs.filter(song => song.artist === 'Saib');
  return saibSongs;
}
...
function getSongsOverThreeMinutes(songs) {
  let longSongs = songs.filter(song => song.runtimeInSeconds > 180);
  return longSongs;
}
...
function getTitleTracks(songs) {
  let sameNames = songs.filter(song => song.title === song.album);
  return sameNames;
}
