...
function getSongsBySaib(songs) {
  let songsBy = songs.filter((song) => song.artist == 'Saib');
    return songsBy
...
function getSongsOverThreeMinutes(songs) {
  let songsOver = songs.filter((song) => song.runtimeInSeconds > 180);
    return songsOver
}
...
function getTitleTracks(songs) {
  let songsTitle = songs.filter((song) => song.title === song.album);
    return songsTitle
}
