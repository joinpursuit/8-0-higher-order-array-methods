...
const getSongsBySaib = (songs) => {
  let songArr = songs.filter((item) => item.artist === "Saib")
  return songArr;
}
...
const getSongsOverThreeMinutes = (songs) => {
  let songArr = songs.filter((item) => item.runtimeInSeconds > 180);
  return songArr;
...
const getTitleTracks = (songs) =>  {
  let songArr = songs.filter((item) => item.title === item.album);
  return songArr;
