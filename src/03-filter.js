...
function getSongsBySaib(songs) {
  let songsBySaib = songs.filter((song) => song.artist === "Saib");
  return songsBySaib;
}
...
function getSongsOverThreeMinutes(songs) {
  let songsOverThreeMinutes = songs.filter(
    (song) => song.runtimeInSeconds / 60 > 3
  );
...
function getTitleTracks(songs) {
  let titleTrack = songs.filter((song) => song.title === song.album);
  return titleTrack;
}
