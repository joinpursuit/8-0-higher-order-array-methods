...
function getSongsBySaib(songs) {
  const filtered = songs.filter((song) => (song.artist === "Saib"));
  return filtered;
}
...
function getSongsOverThreeMinutes(songs) {
  const longerThanThreeMin = songs.filter((song) => (song.runtimeInSeconds > 180));
  return longerThanThreeMin;
}
...
function getTitleTracks(songs) {
  const sameTitleAndAlbum = songs.filter((song) => (song.title === song.album));
