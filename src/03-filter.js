...
function getSongsBySaib(songs) {
  return songs.filter((song) => {return song.artist === 'Saib' ? song : undefined})
}
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter((song) => {return song.runtimeInSeconds > 180 ? song : undefined})
}
...
function getTitleTracks(songs) {
  return songs.filter((song) => {return song.title === song.album ? song : undefined})
}
