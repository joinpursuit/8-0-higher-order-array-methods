...
return songs.filter((song) => song.artist === 'Saib' ? song : false )
}
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter((song) => song.runtimeInSeconds >= 180 ? song : false)
}
...
function getTitleTracks(songs) {
  return songs.filter((song) => song.title === song.album ? song : false)
}
