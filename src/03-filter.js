...
function getSongsBySaib(songs) {
  return songs.filter((song) => (song.artist === "Saib" ? song.title : ""));
}
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter((song) => (song.runtimeInSeconds > 180 ? song.title : ""));
}
...
function getTitleTracks(songs) {
  return songs.filter((song) => (song.album === song.title ? song.title : ""));
}
