...
function getSongTitles(songs) {
 let songTitles = []
  ...
  songs.map(song => {songTitles.push(song.title)})
    return songTitles
}
...
function getSongDetails(songs) {
  let titleByArtistSongs = []

  songs.map(song => {titleByArtistSongs.push(`${song.title} by ${song.artist}`)})
    return titleByArtistSongs
}
...
function getTitleAndArtist(songs) {
  ...
  songs.map(song => {titleByArtistSongs.push({[song.title] : song.artist})})
    return titleByArtistSongs
}
