...
function getSongTitles(songs) {
  let songTitleArr = songs.map((obj)=>{
    return obj.title;
  });
...
function getSongDetails(songs) {
  let songDetailArr = songs.map((obj) =>{
    return obj.title + " by " + obj.artist;
  });
  return songDetailArr;
}
...
function getTitleAndArtist(songs) {
  let titleArtistArr = songs.map((obj) =>{
    return obj = {[obj.title]: obj.artist}
  });
  return titleArtistArr
}
