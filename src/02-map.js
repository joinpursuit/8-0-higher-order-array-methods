 ...
 */
const getSongTitles = songs => songs.map(s => s.title)
 ...
 */
const getSongDetails = songs => songs.map(s => `${s.title} by ${s.artist}`)
 ...
 */
const getTitleAndArtist = songs => songs.map(s => new Object({[s.title]: s.artist}))
