 ...
 */
function getSongTitles(songs) { let songNames = songs.map((song) => { return song.title }); return songNames }
 ...
 */
function getSongDetails(songs) { let songDiscription = songs.map((song) => { return song.title + " by " + song.artist }); return songDiscription }
...
function getTitleAndArtist(songs) {
  ...
  let songArr = songs.map(song => {
    let songObj = {};
    songObj[song.title] = song.artist
    return songObj
  })
  return songArr
}
