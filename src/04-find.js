...
function findPinkElephantsByTimestreet(songs) {
  let foundSong = songs.find(song => song.title === "Pink Elephants")
  return foundSong
}
...
function findFirstSongUnderThreeMinutes(songs) {
  let firstSongUnderThree = songs.find(song => song.runtimeInSeconds < 180)
...
function findFirstTitleTrack(songs) {
  let titleEqualsAlbum = songs.find(song => song.title === song.album)
  return titleEqualsAlbum;
}
