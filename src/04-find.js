...
function findPinkElephantsByTimestreet(songs) {
  return songs.find((song) => song.title === "Pink Elephants" ? song : false)
}
...
function findFirstSongUnderThreeMinutes(songs) {
  return songs.find((song) => song.runtimeInSeconds < 180 ? song : false)
}
...
function findFirstTitleTrack(songs) {
  return songs.find((song) => song.title === song.album ? song : false)
}
