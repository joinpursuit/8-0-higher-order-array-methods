...
function findPinkElephantsByTimestreet(songs) {
  let findPinkEleSong = songs.find(song => song.title === "Pink Elephants")
  return findPinkEleSong
}
...
function findFirstSongUnderThreeMinutes(songs) {
  let newObj = songs.find(song => song.runtimeInSeconds < 180)
  return newObj
}
...
function findFirstTitleTrack(songs) {
  let newObj = songs.find(song => song.title.includes(song.album))
  return newObj
}
