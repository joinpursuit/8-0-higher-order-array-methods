...
function findFirstSongUnderThreeMinutes(songs) {
  function underThreeMins(song) {
    if (song.runtimeInSeconds < 180) {
      return song;
    }
  }
  const find = songs.find(underThreeMins);
  return find;
  //should return on array of objects
  // how do we pull the first one in the list
}
...
function findFirstTitleTrack(songs) {
  return songs.find((song) => {
    ...
    //should only return the first of the area.
  });
}
