...
function findPinkElephantsByTimestreet(songs) {
  const found = songs.find(song => song.title === "Pink Elephants");
  return found
}
...
function findFirstSongUnderThreeMinutes(songs) {
  const found = songs.find(song => song.runtimeInSeconds < 180);
  return found
}
...
function findFirstTitleTrack(songs) {
  const found = songs.find(song => song.title === song.album);
  return found
}
