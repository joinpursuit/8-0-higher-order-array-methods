...
function findPinkElephantsByTimestreet(songs) {
  let song = songs.find((elements) => {
    return elements.title === "Pink Elephants";
  })
  return song;
}
...
function findFirstSongUnderThreeMinutes(songs) {
  let songUnderThreeMin = songs.find((songUnderThreeMin) => {
    return (songUnderThreeMin.runtimeInSeconds < 180);
  });
...
function findFirstTitleTrack(songs) {
  let firstTrack = songs.find((elements) => {
    return elements.title === elements.album;
  });
  return firstTrack;
}
