...
function findPinkElephantsByTimestreet(songs) {
  let pinkElephants = songs.find(song => song.title === 'Pink Elephants');
...
function findFirstSongUnderThreeMinutes(songs) {
  let shortSong = songs.find(song => song.runtimeInSeconds < 180);
  return shortSong;
}
...
function findFirstTitleTrack(songs) {
  let sameSong = songs.find(song => song.title === song.album);
  return sameSong;
}
