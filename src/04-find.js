...
function findPinkElephantsByTimestreet(songs) {
  found = songs.find(song => song.title === "Pink Elephants")
  return found
}
...
function findFirstSongUnderThreeMinutes(songs) {
  return under = songs.find(song => song.runtimeInSeconds < 180)
}
...
function findFirstTitleTrack(songs) {
  return same = songs.find(song => song.title === song.album) 
}
