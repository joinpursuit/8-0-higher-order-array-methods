...
const findPinkElephantsByTimestreet = (songs) => {
    let songArr = songs.find((item) => item.title === "Pink Elephants")
        return songArr;
}
...
const findFirstSongUnderThreeMinutes = (songs) => {
  let songArr = songs.find((item) => item.runtimeInSeconds < 180)
  return songArr;
}
...
const findFirstTitleTrack = (songs) => {
  let songArr = songs.find((item) => item.title === item.album)
  return songArr;
}
