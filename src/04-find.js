...
function findPinkElephantsByTimestreet(songs) {
  let foundElephant = songs.find((song) => song.title === "Pink Elephants");
  return foundElephant;
}
...
function findFirstSongUnderThreeMinutes(songs) {
  let firstSong = songs.find((song) => song.runtimeInSeconds < 180);
  return firstSong;
}
...
function findFirstTitleTrack(songs) {
  let firstSong = songs.find((song) => song.title === song.album);
  return firstSong;
}
