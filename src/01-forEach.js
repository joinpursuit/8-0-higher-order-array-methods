...
function printSongDetails(songs) {
  songs.forEach(songTitleArtist =>{
    console.log(songTitleArtist.title + " by " + songTitleArtist.artist)
...
function printSongTitlesOverThreeMinutes(songs) {
  songs.forEach(runtimeSongs=>{
    if(runtimeSongs.runtimeInSeconds > 180){
      console.log (runtimeSongs.title)
    }
  });
}
