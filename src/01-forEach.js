...
function printAllSongTitles(songs) {
  songs.forEach((obj)=>console.log(obj.title));
}
...
function printSongDetails(songs) {
  songs.forEach((obj)=>console.log(`${obj.title} by ${obj.artist}`));
}
...
function printSongTitlesOverThreeMinutes(songs) {
  songs.filter((obj)=>obj.runtimeInSeconds/60 > 3).forEach((obj)=>console.log(obj.title));
}
