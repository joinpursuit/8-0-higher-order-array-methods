...
function allSongsAreOverTwoMinutes(songs) {
  return songs.every(song => song.title === song.runtimeInSeconds > 160)
};
...
function anySongIsOverFourMinutes(songs) {
  return songs.some(song => song.title === song.runtimeInSeconds > 240 ? true : false)
};
...
function anySongIsByPeanut(songs) {
  return songs.some(song =>  song.artist === 'Peanut' ? true : false)
};
