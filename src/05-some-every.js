...
function allSongsAreOverTwoMinutes(songs) {
  let longSongs = songs.every(song => song.runtimeInSeconds > 120);
  return longSongs;
}
...
function anySongIsOverFourMinutes(songs) {
  let longSong = songs.some(song => song.runtimeInSecons > 240);
  return longSong;
}
...
function anySongIsByPeanut(songs) {
  let songBy = songs.some(song => song.artist === 'Peanut');
  return songBy;
}
