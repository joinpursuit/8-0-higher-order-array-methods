...
function allSongsAreOverTwoMinutes(songs) {
  return songs.every((song) => {return song.runtimeInSeconds > 120 ? true : false})
}
...
function anySongIsOverFourMinutes(songs) {
  return songs.some((song) => {return song.runtimeInSeconds > 240 ? true : false})
}
...
function anySongIsByPeanut(songs) {
  return songs.some((song) => {return song.artist === 'Peanut' ? true : false})
}
