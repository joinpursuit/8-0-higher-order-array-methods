...
function allSongsAreOverTwoMinutes(songs) {
  let songsOverTwoMinutes = songs.every( song => song.runtimeInSeconds > 120)
    return songsOverTwoMinutes
  }
...
function anySongIsOverFourMinutes(songs) {
  let songsOverFourMinutes = songs.some( song => song.runtimeInSeconds > 240)
    return songsOverFourMinutes
  }
...
function anySongIsByPeanut(songs) {
  let songIsByPeanut = songs.some( song => song.artist === 'Peanut')
